/**
 * Unlockt (v6.7) - Web Dashboard & Media Studio Core Engine
 * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
const api = {
    ...
    async getSavedContent(page = 1, type = 'all', sortBy = 'date', dateFrom = null, dateTo = null, hashtag = null, username = null) {
        let url = `/api/saved?page=${page}&limit=${state.itemsPerPage}&type=${type}&sortBy=${sortBy}`;
    ...
    async search(query, searchType, filters = {}) {
    ...
    async downloadItem(itemId, quality = 'high') {
    ...
    async batchDownload(itemIds, quality = 'high') {
        return this.request('/api/download/batch', {
            method: 'POST',
            body: JSON.stringify({ itemIds, quality })
        });
    },
    ...
    if (state.user) {
        ...
        if (elements.userAvatar) {
            elements.userAvatar.onerror = () => {
                elements.userAvatar.src = getLocalAvatarSvg(displayName, 200, 'E1306C', 'fff');
            };
            elements.userAvatar.src = state.user.profilePic || getLocalAvatarSvg(displayName, 200, 'E1306C', 'fff');
        }
    ...
    try {
        ...
        if (response.success) {
            ...
            if (response.user) {
                ...
                elements.userAvatar.src = response.user.profilePic;
...
// Get the proxy URL for a CDN image (with auto-caching via id param)
function getProxyUrl(url, itemId) {
    if (!url) return '';
    if (url.includes('cdninstagram.com') || url.includes('fbcdn.net')) {
        return `/api/proxy-image?url=${encodeURIComponent(url)}${itemId ? `&id=${itemId}` : ''}`;
    }
    return url;
}

// Get the best thumbnail URL for an item
// Priority: cached local (instant) > proxy CDN (auto-caches) > placeholder
function getImageUrl(item) {
    if (!item) return '';
    // Always try local cache first - these load instantly and never expire
    return `/api/thumbnails/${item.id}`;
}
...
function createContentCard(item, navigationList) {
    ...
    // Priority: cached local (instant, never expires) > proxy CDN > placeholder
    const cachedUrl = `/api/thumbnails/${item.id}`;
    const cdnUrl = item.thumbnailUrl || item.mediaUrl || '';
    const proxyUrl = cdnUrl ? getProxyUrl(cdnUrl, item.id) : '';
    // Try cached first (instant if available), then proxy on error
    ...
    card.innerHTML = `
        <div class="content-image-wrapper" style="background: ${placeholderBg};">
            ${thumbnail ?
            `<img src="${thumbnail}" alt="${(item.caption?.substring(0, 50) || 'Saved content').replace(/"/g, '')}" loading="lazy" data-item-id="${item.id}" data-proxy-url="${proxyUrl.replace(/"/g, '&quot;')}">` :
        ...
        ${item.hasAudio && item.audioInfo ? `<span class="content-audio-badge">🎵 ${item.audioInfo.title?.substring(0, 15) || 'Audio'}</span>` : ''}
        <div class="content-overlay">
            <div class="content-user">
                ${item.userProfilePic ? `<img src="${item.userProfilePic}" alt="" class="content-user-pic" onerror="this.style.display='none'">` : ''}
    ...
    // Handle image error - 3-step fallback: cached → proxy → placeholder
    const img = card.querySelector('img[data-item-id]');
    if (img) {
        let triedProxy = false;
        img.onerror = function () {
            // Step 2: Try proxy URL (CDN might still be valid)
            if (!triedProxy && this.dataset.proxyUrl) {
                triedProxy = true;
...
function renderTopAccounts() {
    ...
    accounts.forEach((account, index) => {
        ...
        const fallbackAvatar = getLocalAvatarSvg(account.username, 40, 'E1306C', 'fff');
        el.innerHTML = `
            <img class="account-pic" src="${account.profilePic || fallbackAvatar}" alt="${account.username}"
...
function applyLocalFilters() {
    ...
    // Apply sorting locally for additional sort options
    switch (state.currentSort) {
        case 'date-asc':
            filtered.sort((a, b) => new Date(a.savedAt || a.postedAt) - new Date(b.savedAt || b.postedAt));
            break;
        case 'views':
...
`;
        ...
        const itemsPreview = countItems.slice(0, 50).map(item => {
            const vaultItem = (state.savedContent || []).find(i => 
    ...
    if (isCarousel) {
        ...
        bodyEl.innerHTML = `
                ...
                ${slides.map((slide, idx) => {
                    ...
                    return `
                        <div class="carousel-slide-card selected" id="carouselSlideCard_${idx}" onclick="toggleCarouselSlideSelection(${idx})">
                            ...
                            <button class="carousel-slide-single-dl" title="Download this slide only" onclick="event.stopPropagation(); downloadSingleCarouselSlide(${idx});">
            ...
            <div class="carousel-actions-row">
                ...
                <button class="carousel-action-btn secondary" onclick="downloadSingleCarouselSlide(state.currentSlideIndex || 0)">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16">
                        <circle cx="12" cy="12" r="10"></circle>
...
async function downloadDirectMedia() {
    ...
    triggerDownload(`/api/download-file?id=${item.id}`, filename);
    ...
    const newRecord = {
        ...
        directUrl: `/api/download-file?id=${item.id}`,
        itemsPreview: [{
            id: item.id,
            instagramId: item.instagramId,
            type: item.type,
...
async function downloadSingleCarouselSlide(slideIndex) {
    const item = state.currentInspectedItem || state.currentModalItem;
    if (!item || !item.carouselMedia || !item.carouselMedia[slideIndex]) return;
    ...
    const newRecord = {
        ...
        status: 'completed',
        downloadedAt: new Date().toISOString(),
        thumbnailUrl: slide.thumbnailUrl || slide.imageUrl,
        username: item.username,
        instagramId: item.instagramId,
        caption: (item.caption || '').substring(0, 80),
        itemsPreview: [{
...
async function downloadSelectedCarouselSlidesZip() {
    ...
    for (const idx of selectedIndices) {
        ...
        try {
            const blobRes = await fetch(`/api/proxy-blob?url=${encodeURIComponent(url)}`);
            if (blobRes.ok) {
        ...
        for (const idx of selectedIndices) {
            ...
            await new Promise((resolve) => {
                ...
                img.onerror = () => {
                    ...
                    img.src = `/api/proxy-blob?url=${encodeURIComponent(url)}`;
                    img.onload = () => { loadedImages.push(img); resolve(); };
                    img.onerror = resolve;
                };
                img.src = url.startsWith('http') ? `/api/proxy-blob?url=${encodeURIComponent(url)}` : url;
            });
        }
...
function triggerDownload(url, filename) {
    ...
    // External Instagram CDN URL -> proxy through server to guarantee attachment download
    const proxyUrl = `/api/download-file?url=${encodeURIComponent(url)}&filename=${encodeURIComponent(filename || 'instagram_media')}`;
    const a = document.createElement('a');
    a.href = proxyUrl;
    if (filename) a.download = filename;
    document.body.appendChild(a);
                        ...
                        ${previews.map(preview => {
                            ...
                            return `
                                <div class="dm-preview-item" onclick="openModalFromHistory('${itemId}')" title="${escapeHtml(vaultItem?.caption || preview.caption || authorUser)}">
                                    <img class="dm-preview-thumb" src="${thumbUrl}" alt="@${escapeHtml(authorUser)}" loading="lazy" onerror="this.onerror=null; this.src='/api/thumbnail-proxy?id=${itemId}';">
...
function updateModalContent(item) {
    ...
    if (item.carouselMedia && item.carouselMedia.length > 1) {
        const currentMedia = item.carouselMedia[currentCarouselIndex];
        if (currentMedia.isVideo && currentMedia.videoUrl) {
            // Carousel videos: just try proxy directly (can't refresh specific carousel positions)
            modalVideo.src = `/api/proxy-video?url=${encodeURIComponent(currentMedia.videoUrl)}`;
            modalVideo.classList.remove('hidden');
            modalVideo.play().catch(e => console.log('Carousel video play failed:', e));
        } else {
            // Carousel images: local cache first, then proxy, then fallback
            const carouselImg = currentMedia.imageUrl || currentMedia.thumbnailUrl || '';
            const carouselCacheId = `${item.id}_c${currentCarouselIndex}`;
            const proxyUrl = carouselImg ? getProxyUrl(carouselImg, carouselCacheId) : '';

            modalImage.src = `/api/thumbnails/${carouselCacheId}`;
            let carouselFallback = 0;
            modalImage.onerror = function () {
                carouselFallback++;
                if (carouselFallback === 1 && proxyUrl) {
                    this.src = proxyUrl;
                } else if (carouselFallback === 2) {
                    this.src = `/api/thumbnails/${item.id}`; // Fallback to main thumbnail
                } else {
                    this.onerror = null;
    ...
    } else if (item.type === 'reel' && item.mediaUrl) {
        // Try to play via proxy first, auto-refresh if fails
        playVideoWithRefresh(item, modalVideo, carouselNav);
    } else {
        // Single image: local cache first, proxy, placeholder
        const imgUrl = item.mediaUrl || item.thumbnailUrl || '';
        const proxyUrl = imgUrl ? getProxyUrl(imgUrl, item.id) : '';

        modalImage.src = `/api/thumbnails/${item.id}`;
        let imgFallback = 0;
        modalImage.onerror = function () {
            imgFallback++;
            if (imgFallback === 1 && proxyUrl) {
                this.src = proxyUrl;
            } else {
                this.onerror = null;
                this.src = PLACEHOLDER_IMG;
    ...
    if (userPic) {
        const fallback = getLocalAvatarSvg(item.username || 'U', 40, 'E1306C', 'fff');
        userPic.src = item.userProfilePic || fallback;
...
async function exportSession() {
    showToast('Preparing Master Vault Backup (Content, Downloads, Logs)...', 'info');
    diagLog('INFO', 'EXPORT', 'Initiated full Master Vault backup export');

    try {
        // Trigger download via the export endpoint
        window.location.href = '/api/export';
        showToast('Master Backup download started!', 'success');
        diagLog('SUCCESS', 'EXPORT', `Master Vault backup generated successfully (${state.savedContent.length} items, ${state.downloads.length} downloads)`);
    } catch (error) {
    ...
    fetch(`/api/check-video-cached?id=${itemId}`)
        ...
        .then(data => {
            if (!data.cached) {
                ...
                fetch(`/api/proxy-video?id=${itemId}&url=${encodeURIComponent(videoUrl)}`)
                    .catch(() => {});
...
async function handleThumbnailRefreshed(payload) {
    ...
    // Cache the fresh thumbnail on the server
    try {
        await fetch('/api/cache-thumbnail', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id, url: thumbnailUrl })
        });

        // Update the URL in saved.json
        await fetch('/api/update-thumbnail-url', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id, thumbnailUrl, mediaUrl })
        });
    } catch (e) {
        console.warn('Failed to cache thumbnail:', e);
    }

    // Find and update all visible images for this item across all pages
    const images = document.querySelectorAll(`img[data-item-id="${id}"]`);
    images.forEach(img => {
        // Bust the cache by adding a timestamp
        img.src = `/api/thumbnails/${id}?t=${Date.now()}`;
        img.style.display = '';
...
function refreshThumbnailsForPage() {
    if (isRefreshing) {
        showToast('Already refreshing images...', 'warning');
        return;
    }

    // Collect all items that need refresh from current state
    const items = state.filteredContent || state.savedContent || [];
    if (items.length === 0) {
        showToast('No content to refresh', 'info');
        return;
    }

    // FIRST: try server-side batch caching (doesn't need extension)
    // This downloads thumbnails using current CDN URLs and caches them
    isRefreshing = true;
    const btn = document.getElementById('refreshImagesBtn');
    if (btn) {
        btn.classList.add('refreshing');
        btn.querySelector('.refresh-btn-text').textContent = 'Caching...';
    }

    showToast(`🔄 Caching ${items.length} thumbnails locally...`, 'info');

    // Start server-side batch caching
    fetch('/api/cache-all-thumbnails', { method: 'POST' })
        .then(r => r.json())
        .then(data => {
            if (data.success) {
                // Poll progress
                const progressInterval = setInterval(async () => {
                    try {
                        const prog = await fetch('/api/cache-progress').then(r => r.json());
                        if (prog.progress) {
                            const { cached, failed, total, running } = prog.progress;
                            if (btn) {
                                btn.querySelector('.refresh-btn-text').textContent = `${cached + failed}/${total}`;
                            }
                            if (!running) {
                                ...
                                showToast(`✅ Cached ${cached} thumbnails (${failed} failed)`, cached > 0 ? 'success' : 'warning');
                                // Reload the page content to show cached images
                                state.currentPage = 1;
                                state.savedContent = [];
                                loadSavedContent();
                            }
    ...
    // ALSO: if extension is connected, try refreshing expired URLs
    // SMART REFRESH: Check which items actually need a refresh (not cached yet)
    const allIds = items.map(item => item.id);
    fetch('/api/check-cached-status', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    .then(data => {
        ...
        const itemsNeedingRefresh = items
            .filter(item => item.instagramId && !data.status[item.id])
            .map(item => ({
                id: item.id,
                instagramId: item.instagramId,
        ...
        if (itemsNeedingRefresh.length > 0) {
            console.log(`🧠 Smart Refresh: Only refreshing ${itemsNeedingRefresh.length} uncached items out of ${items.length}`);
            window.postMessage({
                type: 'VAULT_BATCH_REFRESH_THUMBNAILS',
                payload: { items: itemsNeedingRefresh }
            }, '*');
        } else {
            console.log('✅ Smart Refresh: All items are already cached locally!');
        }
    })
    .catch(err => {
        console.warn('Smart refresh check failed, falling back to full refresh:', err);
        // Fallback: refresh all if check fails
        const itemsNeedingRefresh = items
            .filter(item => item.instagramId)
            .map(item => ({
                id: item.id,
                instagramId: item.instagramId,
                mediaId: item.mediaId || item.id
...
function handleDashboardStatClick(statType) {
    ...
    if (statType === 'posts') {
        ...
        if (badgeEl) badgeEl.textContent = `${audioCount} Tracks`;
        if (msgEl) msgEl.innerHTML = `You have <strong>${audioCount} saved audio tracks</strong>. All audio and music saved from Instagram is organized here.`;
        if (actionsEl) {
            actionsEl.innerHTML = `
                <button class="chat-act-btn primary" onclick="navigateToContentFilter('audio')">
                    🎵 Browse Audio (${audioCount})
                </button>
                <button class="chat-act-btn secondary" onclick="showToast('Click Sync New in the sidebar to fetch recent audio tracks', 'info')">
                    🔄 Sync Help
                </button>
            `;
        }
...
const ERROR_DIRECTORY = [
    ...
    },
    {
        ...
        solution: 'The built-in /api/thumbnail-proxy endpoint automatically injects clean referrer headers.',
        fixTool: 'thumbnails',
        checkActive: () => false
    },
    {
    ...
    },
    {
        ...
        solution: 'The system automatically syncs records from /api/download-history.',
        fixTool: 'dlm_sync',
        checkActive: () => false
    },
    {
...
async function runFullSystemDiagnostic() {
    ...
    try {
        ...
        diagLog('ERROR', 'API', 'Failed to reach /api/system-diagnostics: ' + e.message);
    }
    ...
    try {
        ...
        fetch('/api/system-diagnostics/save-scan', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
